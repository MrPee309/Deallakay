...
async def admin_stats(admin: dict = Depends(get_admin)):
    ...
    return {
        ...
        # App engagement — reuses existing users.last_active (set on every
        # /auth/me call, which both the website and mobile app already call
        # on load) rather than a separate tracking system.
        "apk_downloads": (await db.app_stats.find_one({"id": "apk_downloads"}) or {}).get("count", 0),
        "active_users_today": await db.users.count_documents({"last_active": {"$gte": (now - timedelta(days=1)).isoformat()}}),
        "active_users_7d": await db.users.count_documents({"last_active": {"$gte": (now - timedelta(days=7)).isoformat()}}),
        "active_users_30d": await db.users.count_documents({"last_active": {"$gte": (now - timedelta(days=30)).isoformat()}}),
    }
...
async def admin_list_drivers(admin: dict = Depends(require_staff("approve_drivers"))):
    return await db.transport_drivers.find({}, NO_ID).sort("created_at", -1).to_list(500)


@router.put("/transport/drivers/{uid}/{action}")
async def admin_driver_action(uid: str, action: str, admin: dict = Depends(require_staff("approve_drivers"))):
...
async def admin_list_stations(admin: dict = Depends(require_staff("manage_stations"))):
    return await db.transport_stations.find({}, NO_ID).sort("name", 1).to_list(200)


@router.post("/transport/stations")
async def admin_create_station(data: StationIn, admin: dict = Depends(require_staff("manage_stations"))):
    station = {
        "id": str(uuid.uuid4()),
        **data.model_dump(),
        "status": "active",
...
async def admin_update_station(sid: str, data: StationIn, admin: dict = Depends(require_staff("manage_stations"))):
...
async def admin_station_toggle(sid: str, action: str, admin: dict = Depends(require_staff("manage_stations"))):
